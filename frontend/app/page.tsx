    ...
    <div className="flex h-screen overflow-hidden">
      <aside className="w-64 shrink-0 border-r border-[#1a1a1a] flex flex-col p-4 space-y-6 overflow-y-auto scrollbar-thin">
        ...
        <nav className="space-y-1">
          <Link href="/" className="block text-xs text-white bg-[#1a1a1a] px-3 py-2 rounded">◉ Ask</Link>
          <Link href="/timeline" className="block text-xs text-[#666] hover:text-white px-3 py-2 rounded transition-colors">○ Timeline</Link>
          <Link href="/insights" className="block text-xs text-[#666] hover:text-white px-3 py-2 rounded transition-colors">○ Insights</Link>
          <Link href="/analytics" className="block text-xs text-[#666] hover:text-white px-3 py-2 rounded transition-colors">○ Analytics</Link>
        </nav>
        <RepoConnector repos={repos} selectedRepo={selectedRepo} onRepoSelected={setSelectedRepo} onRepoAdded={(r) => setRepos((prev) => [...prev, r])} />
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        {selectedRepo && (
          <div className="border-b border-[#1a1a1a] px-6 py-3 flex items-center justify-between">
            ...
            {loggedCount > 0 && (
              <span className="text-xs text-[#444]">{loggedCount} decision{loggedCount !== 1 ? "s" : ""} logged this session</span>
            )}
          </div>
        )}
